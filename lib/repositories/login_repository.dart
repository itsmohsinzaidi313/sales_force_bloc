...
class LoginRepo {
  ...
  Future<User> login(String email, String password) async {
    ...
    List<Map<String, dynamic>> list = await (await database).query(TableUsers.tableName,
    ...
    if (list.length == 0) {
      ...
      await (await database).update(TableUsers.tableName, {TableUsers.loginStatus: '1'},
  ...
  Future<bool> logout() async {
    await (await database).update(TableUsers.tableName, {TableUsers.loginStatus: '0'});
    return true;
  }

  Future<User> getLastLogin() async {
    List<Map<String, dynamic>> list = await (await database).query(TableUsers.tableName,
        where: '${TableUsers.loginStatus} = ?', whereArgs: [1]);
    if (list.isEmpty) {
      return null;
    } else {
